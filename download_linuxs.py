...
from techmaster import url_linux, cookies, path_linux

requests = retry(requests.Session(), retries=5, backoff_factor=0.2)

res = requests.get(url=url_linux, cookies=cookies)
data = json.loads(res.content)
sections = data['Sections']

for index_section, section in enumerate(sections):

    path_section = path_linux + str(index_section) + '_' \
    ...
    for index_lesson, lesson in enumerate(section['lessons']):
        ...
        url = 'https://techmaster.vn/user/learn/126/25537/{id}/{first_step}' \
